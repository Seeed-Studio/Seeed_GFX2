...
Seeed_GFX display;            // Invoke custom library with default width and height
Seeed_Sprite spr;  // Declare Sprite object "spr" with pointer to "tft" object
...
void setup(void) {
  ...
  //display.setViewport(0, 0, 240, 320);
}
...
void loop() {
  ...
  static int16_t xpos = display.width() / 2;
  static int16_t ypos = display.height() / 2;
  bool newMeter = false;

  if (maxRadius == 0) {
    maxRadius = display.width();
    if (display.height() < maxRadius) maxRadius = display.height();
  ...
  display.fillCircle(xpos, ypos, radius + 1, TFT_NAVY);
...
void ringMeter(int x, int y, int r, int val, const char *units)
{
  ...
  if (initMeter) {
    initMeter = false;
    last_angle = 30;
    display.fillCircle(x, y, r, DARKER_GREY);
    display.drawSmoothCircle(x, y, r, TFT_SILVER, DARKER_GREY);
    uint16_t tmp = r - 3;
    display.drawArc(x, y, tmp, tmp - tmp / 5, last_angle, 330, TFT_BLACK, DARKER_GREY);
  }
  ...
  if (last_angle != val_angle) {
    ...
    if ( r >= 25 ) {
      ...
      if (!spr.createSprite(&display, w, h + 2)) {
        Serial.println("Meter label sprite allocation failed");
        return;
      }
    ...
    if (val_angle > last_angle) {
      display.drawArc(x, y, r, r - thickness, last_angle, val_angle, TFT_SKYBLUE, TFT_BLACK); // TFT_SKYBLUE random(0x10000)
    }
    else {
      display.drawArc(x, y, r, r - thickness, val_angle, last_angle, TFT_BLACK, DARKER_GREY);
    }
