...
Seeed_Sprite graph1; // Sprite object graph1

Seeed_Sprite stext1; // Sprite object stext1

Seeed_Sprite stext2; // Sprite object stext2
...
void setup() {
  ...
  if (!graph1.createSprite(&display, 128, 61)) {
    Serial.println("Graph sprite allocation failed");
    while (true) delay(1000);
  }
  ...
  if (!stext1.createSprite(&display, 32, 64)) {
  ...
  if (!stext2.createSprite(&display, 80, 16)) {
    Serial.println("Text sprite allocation failed");
    while (true) delay(1000);
  }
