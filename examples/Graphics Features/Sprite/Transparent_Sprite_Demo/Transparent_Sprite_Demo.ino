...
Seeed_Sprite img;  // Create Sprite object "img" with pointer to "tft" object
...
void loop() {
  ...
  for (int i = 0; i < 10; i++)
  {
    int x = random(display.width()-70);
    int y = random(display.height()-80);
...
void drawStar(int x, int y, int star_color)
{
  ...
  img.setColorDepth(8);
  if (!img.createSprite(&display, 70, 80)) return;
...
void numberBox(int x, int y, float num )
{
  ...
  img.setColorDepth(8);
  if (!img.createSprite(&display, IWIDTH, IHEIGHT)) return;
