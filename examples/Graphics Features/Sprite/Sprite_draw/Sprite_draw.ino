...
#include "panel/Panel_TFT.h"                 // Include the graphics library (this includes the sprite functions)
...
void loop(void)
{
  ...
  spr.pushSprite(display.width() - WIDTH + 40, display.height() - HEIGHT + 40);
  ...
  while (true)
  {
    ...
    if (x < -WIDTH/2) dx = 1;
    if (x >= display.width()-WIDTH/2) dx = -1;
    if (y < -HEIGHT/2) dy = 1;
    if (y >= display.height()-HEIGHT/2) dy = -1;
