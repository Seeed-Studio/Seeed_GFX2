...
// Width and height of sprite: 160x160 image plus a 2-pixel border.
#define WIDTH  164
#define HEIGHT 164
...
Seeed_GFX display;         // Declare object "tft"
...
void setup()
{
  ...
  if (!spr.createSprite(&display, WIDTH, HEIGHT)) {
    Serial.println("4-bit sprite allocation failed");
    while (true) delay(1000);
  }
  ...
  // A 4-bit sprite interprets the source as two packed palette indices/byte.
  spr.pushImage(2, 2, 160, 160, (const uint16_t *)stars);
...
void loop(void)
{
  ...
  spr.pushSprite(display.width() / 2 - WIDTH / 2, display.height() / 2 - HEIGHT / 2);
