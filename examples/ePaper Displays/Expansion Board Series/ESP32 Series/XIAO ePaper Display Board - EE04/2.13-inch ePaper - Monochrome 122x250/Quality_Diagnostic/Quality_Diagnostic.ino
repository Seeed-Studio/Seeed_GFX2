/**
 * Same-panel A/B quality diagnostic: 2.13-inch SSD1680 on XIAO EE04.
 *
 * Use this sketch and the Breakout counterpart with the SAME panel, ambient
...
void setup() {
    ...
    if (!display.begin<Board_XIAO_ePaper_EE04,
                       Config_Seeed_ePaper_2inch13_BW_SSD1680>()) {
        Serial.println(display.lastResult().message);
        return;
    }
    display.setRotation(3);
    Serial.printf("board=EE04 driver=%s w=%u h=%u rot=%u\n",
                  display.driverPtr()->name(), display.width(), display.height(),
                  display.getRotation());
    drawQualityPattern("EE04");
