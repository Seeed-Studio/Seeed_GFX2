 ...
 *
 * The 2.9" glass is physically landscape (long edge horizontal). The SSD1680
 * controller RAM is natively portrait (128 x 296), so we rotate the frame
 * buffer into a 296 x 128 landscape viewport with setRotation(3). Text then
 * reads along the long edge instead of sideways, and the wide canvas fits the
 * demo content without clipping.
 */
#include <Seeed_GFX.h>

Seeed_GFX display(Seeed_Product::Seeed_ePaper_2INCH9);
...
void setup() {
    ...
    // reads along the long edge of the glass.
    ...
    display.drawRightString("SSD1680", w - 8, 10, 1);
