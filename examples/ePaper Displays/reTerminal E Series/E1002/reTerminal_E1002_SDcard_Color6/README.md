...
将 SD 卡上的 JPG/BMP/PNG 图片显示到 reTerminal E1002 的 7.5 英寸 6 色电子墨水屏（800×480，ED2208）。使用统一的 `dither/Dither.h` 模块和 `DitherConfig` API，以编译期常量暴露完整的抖动参数集（初始值全部为库默认值），支持 7 种抖动/量化方法。图片自动缩放到 800×480 全屏显示。
