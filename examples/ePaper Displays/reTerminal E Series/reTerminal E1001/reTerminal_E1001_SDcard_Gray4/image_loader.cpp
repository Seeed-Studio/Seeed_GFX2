...
static void pngle_on_done(pngle_t* p) {
  PngCtx* ctx = (PngCtx*)pngle_get_user_data(p);
  ctx->done = true;
}
...
static bool decode_png(File& f, RgbImage* out) {
  ...
  PngCtx ctx{ out, false, false };
  pngle_set_user_data(png, &ctx);
  pngle_set_init_callback(png, pngle_on_init);
  pngle_set_draw_callback(png, pngle_on_draw);
  pngle_set_done_callback(png, pngle_on_done);

  // pngle can stop just before a split chunk header/CRC and return zero.
  // Preserve that tail and append the next SD read instead of overwriting it.
  constexpr size_t CHUNK = 4096;
  constexpr size_t TAIL  = 64;
  uint8_t buf[CHUNK + TAIL];
  size_t buffered = 0;
  bool err = false;
  while (!ctx.done) {
    if (buffered == sizeof(buf)) {
      Serial1.println("[png] decoder stalled on an oversized header");
      err = true;
      break;
    }

    const size_t room = sizeof(buf) - buffered;
    const size_t request = room < CHUNK ? room : CHUNK;
    const int n = f.read(buf + buffered, request);
    if (n < 0) {
      Serial1.println("[png] SD read failed");
      err = true;
      break;
    }
    if (n == 0 && buffered == 0) break;

    const size_t total = buffered + (size_t)n;
    size_t offset = 0;
    while (offset < total) {
      const int eaten = pngle_feed(png, buf + offset, total - offset);
      if (eaten < 0) {
    ...
    if (buffered) memmove(buf, buf + offset, buffered);
    if (n == 0) break;
  }

  if (!err && !ctx.done) {
    Serial1.println("[png] truncated file: IEND was not received");
    err = true;
  }
  const bool ok = !err && !ctx.oom && ctx.done && out->pixels != nullptr;
  pngle_destroy(png);
  if (!ok) {
