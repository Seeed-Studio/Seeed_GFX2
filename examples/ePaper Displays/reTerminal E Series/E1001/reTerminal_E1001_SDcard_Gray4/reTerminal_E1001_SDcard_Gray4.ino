 ...
 *
 * UC8179 is a BW controller, but the library supports PanelMode::Gray4,
 * which gives four shades (black, dark gray, light gray, white).
 *
 * Pipeline mirrors online_img2bitmap_.html with screen=gray4:
 *   SD JPG/BMP/PNG -> RGB888 -> luma + gamma + Floyd-Steinberg gray4 dither
 *               -> packed 4bpp -> pushImage4BPP into the Gray4 frame buffer.
 *
 ...
 *           SD  EN       16
 *           SD  DET      15
 *           SD  MISO     8   (routed by the library E1001 board config)
 *
...
static bool show_image_on_panel(RgbImage* img) {
  ...
  pack_4bpp_in_place(idx, W, H, 0x03); // Gray4 padding must be white.

  // 5) Preserve the packed Gray4 nibbles; this is not an RGB565 image.
...
void setup() {
  ...
  const GfxResult modeResult = display.panel().configure(PanelMode::Gray4);
  if (!modeResult) {
    LOG.printf(TAG " Gray4 configuration failed: %s\n", modeResult.message);
    return;
  }
  // configure(Gray4) allocates the 4bpp buffer and initializes it to index 3
  // (white). A preliminary physical white refresh is intentionally omitted.
  log_mem("after Gray4 configure");
